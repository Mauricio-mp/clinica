...
function BusquedaNuevo (){
    ...
    init:function(){$("#tabla_registro").DataTable(
      {
        ...
        { targets: -2, 
          ...
          { 
              var s = { 
                   1: { title: "Pendiente", state: "primary" },
    ...
    .done(function(data) {
       ...
       doctor=json;
      for (const index = 0; index <= json.length; index++) {
        console.log(index);
        
      }

       $('#cbxDocotres').append($('<option>').val(json['data'][0]['id_usuario']).text(json['data'][0]['nombrecompleto']));
